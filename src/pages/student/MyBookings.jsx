...
const TABS = ['all', 'pending', 'upcoming', 'completed', 'cancelled', 'rejected', 'released']
const PAGE_SIZE = 10
...
// pending / rejected / cancelled / released show as-is; confirmed splits by time
function getDisplayStatus(booking) {
  if (booking.status === 'confirmed') {
    return new Date(booking.end_time) > new Date() ? 'upcoming' : 'completed'
  }
...
export default function MyBookings() {
  ...
  const [counts, setCounts]       = useState({
    pending: 0, upcoming: 0, completed: 0, cancelled: 0, rejected: 0, released: 0,
  })
  ...
  async function getBookings(pageNum = 1, reset = false) {
    ...
    let query = supabase
      .from('bookings')
      .select('*, facilities(facility_name, location, booking_mode)')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false })
      .range((pageNum - 1) * PAGE_SIZE, pageNum * PAGE_SIZE - 1)

    if (activeTab === 'pending')        query = query.eq('status', 'pending')
    else if (activeTab === 'upcoming')  query = query.eq('status', 'confirmed').gt('end_time', now)
    else if (activeTab === 'completed') query = query.eq('status', 'confirmed').lte('end_time', now)
    else if (activeTab === 'cancelled') query = query.eq('status', 'cancelled')
    else if (activeTab === 'rejected')  query = query.eq('status', 'rejected')
    else if (activeTab === 'released')  query = query.eq('status', 'released')
  ...
  async function getCounts() {
    if (!user) return
    const now = new Date().toISOString()
    const own = () => supabase.from('bookings').select('booking_id', { count: 'exact', head: true }).eq('user_id', user.id)

    const [pending, upcoming, completed, cancelled, rejected, released] = await Promise.all([
      own().eq('status', 'pending'),
      own().eq('status', 'confirmed').gt('end_time', now),
      own().eq('status', 'confirmed').lte('end_time', now),
      own().eq('status', 'cancelled'),
      own().eq('status', 'rejected'),
      own().eq('status', 'released'),
    ])
  ...
  async function handleCancel(bookingId) {
    ...
    if (!error) {
      ...
      await supabase.from('notifications').insert({
        ...
        message:    `Your booking for ${formatFacilityName(b?.facilities?.facility_name)} has been cancelled. Tap to rebook the same slot.`,
        type:       'booking',
      })
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 w-full">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          {[
            ...
            { label: 'Released',  count: counts.released,  color: 'text-orange-600', tab: 'released' },
          ].map(stat => (
            <div
              key={stat.tab}
              onClick={() => setActiveTab(stat.tab)}
              className={`flex-1 min-w-[110px] bg-white rounded-xl shadow-sm p-4 text-center cursor-pointer hover:shadow-md transition border-2 ${
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading && bookings.length === 0 ? (
                ...
                <tr><td colSpan={5} className="text-center py-12 text-gray-400 text-sm">No bookings found.</td></tr>
              ) : (
                  ...
                  return (
                    <tr key={booking.booking_id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                      <td className="px-6 py-5">
                        <p className="text-sm font-medium text-gray-900">{formatFacilityName(booking.facilities?.facility_name)}</p>
